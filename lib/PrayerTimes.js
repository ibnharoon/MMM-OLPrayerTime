...
class PrayerTime {
  constructor(date, latitude, longitude, language, timeFormat, timezone) {
    this.date = new Dayjs(date).add(3, 'hour'); // add 3 hour from midnight to account for dst
    ...
    this.dst = this.isInDaylightSavingTime(this.date.toDate());
    this.prayertime = new PrayerTimes();
    this.prayertime.setMethod('ISNA');
    this.prayertime.adjust({ asr: 'Standard' });
    console.log('current date: ' + this.date.toDate() + ', dst:' + this.dst);
  ...
  calculateDelta() {
    Prayers.forEach((prayer, index) => {
      ...
      if (prayer === 'Midnight') {
        ...
        this.times[prayer]['yhijri'] = this.times[prayer]['ymidnight'].add(1, 'day').toCalendarSystem('hijri').locale(this.language).format('D MMM YYYY');
      } else {

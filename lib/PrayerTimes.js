...
class PrayerTime {
  ...
  calculateDelta() {
    Prayers.forEach((prayer, index) => {
      ...
      if (prayer === 'Midnight') {
        ...
        console.log('calculateDelta() yesterday\'s midnight time: ' + this.times[prayer]['ymidnight']);
        var addday = 0;
  ...
  getPrayerTimes(date) {
    ...
    const dstdate = this.date.set('hour', 0).set('minute', 0).set('second', 0).set('millisecond', 0).add(3, 'hour');  // add 3 hour from midnight to account for dst
    ...
    return Prayers.reduce((acc, prayer) => {
      ...
      console.log('prayer: ' + prayer + ', before: ' + acc[prayer]['time'].toDate());

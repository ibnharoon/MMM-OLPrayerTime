...
class PrayerTime {
  constructor(date, latitude, longitude, language, timeFormat, timezone) {
    ...
    this.dst = this.isInDaylightSavingTime(date);
    ...
    this.date = new Dayjs.tz(date, this.timezone);
    // this.date = new Dayjs(date);
    console.log('current date: ' + this.date.tz(this.timezone).toDate());
  ...
  getPrayerTimes(date) {
    ...
    let dayTimes = this.prayertime.getTimes(date.toDate(), [this.latitude, this.longitude], this.dst ? -8 : -7, this.dst);
    console.log('daytimes: ' + JSON.stringify(dayTimes));
    return Prayers.reduce((acc, prayer) => {
      acc[prayer] = {};
      acc[prayer]['time'] = new Dayjs(`${date.format('YYYY-MM-DD')} ${dayTimes[prayer.toLowerCase()]}`, 'YYYY-MM-DD HH:mm a').tz(this.timezone);
      // handle cases where midnight time is after real midnight
      console.log('before: ' + acc[prayer]['time'].tz(this.timezone).toDate());
      ...
      acc[prayer]['stime'] = acc[prayer]['time'].tz(this.timezone).format(fstring);
      // console.log('after: ' + acc[prayer]['time'].tz(this.timezone).toDate());

...
Module.register("MMM-OLPrayerTime", {
      ...
      for (cp in this.prayerInfo['timings']) {
        ...
        if (this.updateptimes || this.updatecprayer) {
          ...
          if (this.updateptimes) {
            ...
            prayerTime.innerHTML = this.prayerInfo['timings'][cp];;
          }
        }
      }
  ...
  updateDuration: function (uptime, ucprayer) {
    ...
    if (this.currentPrayer['nprayer'] != null) {
      ...
      if (nptime >= dnow) {
        ...
        this.delta = Math.floor((nptime - dnow) / 60000);
  ...
  socketNotificationReceived: function (notification, payload) {
    ...
    switch (notification) {
      ...
      case 'UPDATECURRENTPRAYER':
        ...
        this.updateDuration(false, true);
        this.dateUpdated = false;
        break;
  ...
  notificationReceived: function (notification, payload, sender) {
    switch (notification) {
      case 'CLOCK_MINUTE':
        ...
        if (this.delta > 0) {
          this.delta -= 1;
          this.updateDuration(false, false);
        }

        if (!this.dateUpdated) {
          var nextDay = (['maghrib', 'isha'].includes(this.currentPrayer['cprayer'])) ? true : false;
          this.sendSocketNotification('GETDATE', { 'nextday': nextDay });
        }
        break;
    }
  }
